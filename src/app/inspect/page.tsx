...
export default function InspectLandingPage() {
  return (
    <div className="-mx-4 -mt-4 min-h-screen bg-slate-950 px-4 py-8 sm:-mx-6 lg:-mx-8">
      <div className="mx-auto w-full max-w-6xl pb-12 pt-6 sm:px-2 lg:pt-12">
        <section className="rounded-[2rem] border border-white/10 bg-slate-900/50 p-6 shadow-sm sm:p-10">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-teal-400">
            Vehicle Inspection
          </p>
          <h1 className="mt-4 max-w-2xl text-3xl font-black tracking-[-0.06em] text-white sm:text-5xl">
          ...
          {MODES.map((mode) => {
            ...
            return (
              <Link
                key={mode.href}
                href={mode.href}
                className="group relative flex flex-col rounded-[2rem] border border-white/10 bg-slate-950/68 p-6 shadow-panel transition hover:border-teal-400/60 hover:bg-[#0b1326]/84 hover:shadow-lg sm:p-8"
              >
                {mode.badge && (
                ...
                <div className="inline-flex self-start rounded-2xl bg-teal-500/10 p-4 text-teal-400 border border-teal-400/20">
                ...
                <p className="mt-1 text-sm font-bold text-teal-400">{mode.subtitle}</p>
                <p className="mt-3 text-sm leading-7 text-slate-400">{mode.description}</p>
                ...
                <div className="mt-6 inline-flex items-center gap-2 text-sm font-black text-teal-400 group-hover:gap-3 transition-all">
                  {mode.cta}
